...
---------------------------------------------`);
        ...
        if (!this.eventTypeIsValid(gitEventType)) {
            ...
            throw new NotFoundException(
                `Event Type ${gitEventType} was not found/supported`
            );
        }

        const result = await this.webhooksService.handleEvent(
            id,
            gitEventType,
            body
        );
    ...
    private eventTypeIsValid(eventType: string): boolean {
        return (
            Object.keys(GitHubWebhookEvents).find(
                val => GitHubWebhookEvents[val] === eventType
            ) != null
        );
    }
}
