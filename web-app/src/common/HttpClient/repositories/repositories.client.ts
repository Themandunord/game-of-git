	...
	public async retrieveUsersStoredRepositories() {
		try {
			const reposResponse = await this.client.get(
				`${AbstractHttpClient.apiUrl}/repositories/${this.client.user.id}/get-tracked-repositories`
			);
			if (reposResponse.status === 200) {
				return reposResponse.data;
			}
			throw new Error(
				`Error querying for repositories store sync: ${reposResponse.statusText}`
			);
		} catch (e) {
			console.error('There was an error retrieving users stored repositories: ', e);
		}
	}
