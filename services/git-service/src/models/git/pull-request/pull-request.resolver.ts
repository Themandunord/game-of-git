...
import { GitHubCommit, GitHubPullRequest } from 'src/graphql.schema';
...
export class PullRequestResolver {
	...
	async getGitHubPullRequests(@Args() args, @Info() info): Promise<GitHubPullRequest[]> {
		return await this.prisma.query.gitHubPullRequests(args, info);
	}

	@Query('gitHubPullRequest')
	async getGitHubPullRequest(@Args() args, @Info() info): Promise<GitHubPullRequest> {
		return await this.prisma.query.gitHubPullRequest(args, info);
	}

	@Mutation('createGitHubPullRequest')
	async createGitHubPullRequest(@Args() args, @Info() info): Promise<GitHubPullRequest> {
		return await this.prisma.mutation.createGitHubPullRequest(args, info);
	}

	@Mutation('updateGitHubPullRequest')
	async updateGitHubPullRequest(@Args() args, @Info() info): Promise<GitHubPullRequest> {
