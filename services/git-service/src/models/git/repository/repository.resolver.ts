...
import { GitHubRepository } from 'src/graphql.schema';
...
export class RepositoryResolver {
	...
	async getGitHubRepositories(@Args() args, @Info() info): Promise<GitHubRepository[]> {
		return await this.prisma.query.gitHubRepositories(args, info);
	}

	@Query('gitHubRepository')
	async getGitHubRepository(@Args() args, @Info() info): Promise<GitHubRepository> {
		return await this.prisma.query.gitHubRepository(args, info);
	}

	@Mutation('createGitHubRepository')
	async createGitHubRepository(@Args() args, @Info() info): Promise<GitHubRepository> {
		return await this.prisma.mutation.createGitHubRepository(args, info);
	}

	@Mutation('updateGitHubRepository')
	async updateGitHubRepository(@Args() args, @Info() info): Promise<GitHubRepository> {
