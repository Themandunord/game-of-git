...
import { GitHubCommit, GitHubPullRequestReview } from 'src/graphql.schema';
...
export class PullRequestReviewResolver {
	...
	async getGitHubPullRequestReviews(@Args() args, @Info() info): Promise<GitHubCommit[]> {
		return await this.prisma.query.gitHubCommits(args, info);
	}

	@Query('gitHubPullRequestReview')
	async getGitHubPullRequestReview(@Args() args, @Info() info): Promise<GitHubPullRequestReview> {
