...
import { GitHubUser, GitHubComment } from 'src/graphql.schema';
...
export class CommentResolver {
	...
	async getGitHubComments(@Args() args, @Info() info): Promise<GitHubComment[]> {
	...
	async createGitHubComment(@Args() args, @Info() info): Promise<GitHubComment> {
		return await this.prisma.mutation.createGitHubComment(args, info);
	}

	@Mutation('updateGitHubComment')
	async updateGitHubComment(@Args() args, @Info() info): Promise<GitHubComment> {
