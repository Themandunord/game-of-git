...
import { GitHubReviewRequest } from 'src/graphql.schema';

@Resolver('ReviewRequest')
export class ReviewRequestResolver {
	constructor(private readonly prisma: PrismaService) {}

	@Query('gitHubReviewRequests')
	async getGitHubReviewRequests(@Args() args, @Info() info): Promise<GitHubReviewRequest[]> {
		return await this.prisma.query.gitHubReviewRequests(args, info);
	}

	@Query('gitHubReviewRequest')
	async getGitHubReviewRequest(@Args() args, @Info() info): Promise<GitHubReviewRequest> {
		return await this.prisma.query.gitHubReviewRequest(args, info);
	}

	@Mutation('createGitHubReviewRequest')
	async createGitHubReviewRequest(@Args() args, @Info() info): Promise<GitHubReviewRequest> {
		return await this.prisma.mutation.createGitHubReviewRequest(args, info);
	}

	@Mutation('updateGitHubReviewRequest')
	async updateGitHubReviewRequest(@Args() args, @Info() info): Promise<GitHubReviewRequest> {
