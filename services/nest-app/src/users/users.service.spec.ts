...
const usersResolverMock = jest.genMockFromModule<UsersResolver>(
  "./users.resolver"
);
...
describe("UsersService", () => {
  ...
  describe("Retrieving a user by ID", () => {
    it("Calls UsersResulver.getUser once and returns the correctly shaped result", async () => {
      ...
      await expect(service.getById("someID1234")).resolves.toEqual(
        expectedUserData
      );
  ...
  it("Retrieving a user by email", async () => {
    ...
    await expect(service.getByEmail("someemail")).resolves.toEqual(
      expectedUserData
    );
    expect(getUserMock).toHaveBeenCalledTimes(1);
  });
