...
export class AuthController {
  ...
  async login(@Body("email") email, @Body("password") password: string) {
    this.logger.log(
      `Auth Controller: Trying to login for ${email} with ${password}`
    );
    ...
    this.logger.log(
      `Auth Controller: Valid user ${email}, return authentication token.`
    );
  ...
  async refresh(@Body("jwt") jwt) {
    this.logger.log(
      "Request to refresh jwt token, check if it is a valid token"
    );
