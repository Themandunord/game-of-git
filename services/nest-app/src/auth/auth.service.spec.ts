...
const usersServiceMock = jest.genMockFromModule<UsersService>(
  "./../users/users.service"
);
const jwtServiceMock = jest.genMockFromModule<JwtService>(
  "@nestjs/jwt/dist/jwt.service"
);
...
describe("AuthService", () => {
  ...
  describe("Encrypting and comparing a User Password", () => {
    it("Can Successfully encrypt and validate a password and hashed token", async () => {
      ...
      await expect(
        service.comparePassword(password, hashed)
      ).resolves.toBeTruthy();
    });
  });
});
